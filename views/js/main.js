...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    var randomPizzas = document.getElementsByClassName('randomPizzaContainer');//Optimizing for loop.
    var i = 0;
    var dx = determineDx(randomPizzas[i], size);
    var newwidth = (randomPizzas[i].offsetWidth + dx) + 'px';
    for (; i < randomPizzas.length;) {
      randomPizzas[i].style.width = newwidth; i++;
    }
  }
...
function updatePositions() { //optimizing the for loop.
  frame++;
  ...
  for(; i < l;){
    ...
    array.push(phase);
    phase = array[i % 5];
    items[i].style.left = items[i].basicLeft + 100 * array[i] + 'px'; i++;
  }
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256, i = 0;//optimizing the for loop
  var intFrameHeight = window.innerHeight;
  var num = intFrameHeight/s*cols;//num == number of animating pizza's generated in the background 
  for (; i < num;) {
    ...
    document.querySelector("#movingPizzas1").appendChild(elem); i++;
  }
  items = document.getElementsByClassName('mover');//replaced dovument.querySelectorAll.
  requestAnimationFrame(updatePositions);//To avoid frame missing.
});
